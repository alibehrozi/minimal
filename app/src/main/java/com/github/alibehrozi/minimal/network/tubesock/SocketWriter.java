...
class SocketWriter {
    ...
    private final BlockingQueue<ByteBuffer> pendingBuffers;
    ...
    SocketWriter(Socket websocket, String threadBaseName, int clientId) {
        innerThread = Socket.getThreadFactory().newThread(this::runWriter);

        Socket.getIntializer().setName(getInnerThread(), threadBaseName + "Writer-" + clientId);
    ...
    synchronized void send(byte opcode, boolean masking, byte[] data) throws IOException {
        ...
        if (stop && (closeSent || opcode != Socket.OPCODE_CLOSE)) {
            throw new SocketException("Shouldn't be sending");
        }
    ...
    private void runWriter() {
        try {
            ...
            handleError(new SocketException("IO Exception", e));
