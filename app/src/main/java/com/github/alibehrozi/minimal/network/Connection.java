...
public class Connection implements SocketConnection.Delegate {
    ...
    public void onMessage(Map<String, Object> message) {
        try {
            ...
            if (messageType == null) {
                if (logger.logsDebug())
                    logger.debug(
                            "Failed to parse server message: missing message type:" + message);
                close();
                return;
            }
            ...
            if (logger.logsDebug()) logger.debug("Failed to parse server message: " + e);
            close();
        }
    }
    ...
    private void onHandshake(Map<String, Object> handshake) {
        if (handshake == null) {
            if (logger.logsDebug()) logger.debug("Failed to handshake ");
            close();
            return;
        }
        if (!handshake.containsKey(SERVER_HELLO_TIMESTAMP)) {
            if (logger.logsDebug()) logger.debug("Failed to get \"SERVER_HELLO_TIMESTAMP\".");
            close();
            return;
        }
