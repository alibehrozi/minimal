...
import com.github.alibehrozi.minimal.network.core.JsonMapper;
import com.github.alibehrozi.minimal.network.tubesock.Socket;
import com.github.alibehrozi.minimal.network.tubesock.SocketEventHandler;
import com.github.alibehrozi.minimal.network.tubesock.SocketException;
import com.github.alibehrozi.minimal.network.tubesock.SocketMessage;
...
class SocketConnection {
    ...
    private class WSClientTubesock implements WSClient, SocketEventHandler {

        private final Socket ws;

        private WSClientTubesock(Socket ws) {
            this.ws = ws;
        ...
        @Override
        public void onMessage(SocketMessage msg) {
            final String str = msg.getText();
            if (logger.logsDebug()) logger.debug("ws message: " + str);
            executorService.execute(() -> handleIncomingFrame(str));
        }
        ...
        @Override
        public void onError(final SocketException e) {
            executorService.execute(
                    () -> {
    ...
    private WSClient createConnection(
        ...
        Socket ws = new Socket(connectionContext, uri, /*protocol=*/ null);
        // todo : WSClientTubesock add to list of connection
